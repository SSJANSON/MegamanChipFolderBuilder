...
const ChipDetails = (props) => {
    ...
    useEffect(() => {
        async function getCurrentChip() {
            const newChip = await chips.find(chip => ((chip.name === detail.name) && (chip.letter === detail.letter)))
            setCurr(newChip)
          }
          getCurrentChip()
    }, [detail]);
    ...
    return (
        <div>
            ...
            <div className="chip-info" style={{ backgroundImage:`url(${image})`, backgroundRepeat:"no-repeat"}}>
                {currentChip && <div>
                    <img className="chip-info-pic" src={`/Images/MMBN6/${currentChip.category}${currentChip.chip_id}${currentChip.ver}.webp`} alt="image"></img>
